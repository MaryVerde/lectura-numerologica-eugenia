...
if clave_ingresada:
    ...
    if telefono.strip():
        ...
        st.markdown('<div class="em-card em-muted">📞 Si deseas, agrega un teléfono para activar esta lectura.</div>', unsafe_allow_html=True)

    # Hogar / Dirección
    if direccion_apto.strip():
        num_dir = numero_apto(direccion_apto)
        ...
        st.markdown('<div class="em-card em-muted">🏠 Si deseas, agrega tu dirección o número de apto para activar esta lectura.</div>', unsafe_allow_html=True)
    ...
    secciones_completa = [
        ...
        ("Teléfono", f"Número {num_tel if num_tel else '—'}\n\n{TEXTO_TELEFONO.get(num_tel) if num_tel else 'No se ingresó teléfono.'}"),
        ("Dirección / Apto", f"Número {num_dir if num_dir else '—'}\n\n{TEXTO_HOGAR.get(num_dir) if num_dir else 'No se ingresó dirección/apto.'}"),
