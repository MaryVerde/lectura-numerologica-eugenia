...
# ===============================
# ESTADO PREMIUM GLOBAL (OBLIGATORIO)
# ===============================
if "es_premium" not in st.session_state:
    st.session_state["es_premium"] = False

es_premium = st.session_state["es_premium"]

## =====================================================
# 🔐 CONFIGURACIÓN DE CLAVES (VS CODE / STREAMLIT CLOUD)
# =====================================================

def get_secret(key: str, default: str = "") -> str:
    """
    Devuelve una clave desde:
    1) Streamlit Cloud (st.secrets)
    2) Variables de entorno
    3) Default local (VS Code)
    """
    try:
...
# 🔑 Claves reales
APP_SECRET   = get_secret("APP_SECRET", "Marana72")
ADMIN_SECRET = get_secret("ADMIN_PIN", "Marana72")

if not APP_SECRET:
    st.error("❌ Falta configurar APP_SECRET")
    st.stop()
...
# ===============================
# 📊 CONTADOR LECTURA GRATUITA
# ===============================

if "contador_gratis" not in st.session_state:
    st.session_state["contador_gratis"] = 0

# Incrementa UNA SOLA VEZ por sesión
if "registro_gratis" not in st.session_state:
    st.session_state["contador_gratis"] += 1
    st.session_state["registro_gratis"] = True

st.caption(f"👥 Personas que han accedido a la lectura gratuita: {st.session_state['contador_gratis']}")
 #=====================================================
...
# ===============================
# 🔐 ACCESO ADMIN (CLAVE MAESTRA)
# ===============================
st.markdown("### 🔐 Acceso Administrador")

admin_key = st.text_input(
    "Clave maestra (solo Eugenia)",
    type="password"
)

es_admin = False
if admin_key:
    if hmac.compare_digest(admin_key, ADMIN_SECRET):
        es_admin = True
        st.success("✅ Acceso administrador activo")
    else:
        st.error("❌ Clave maestra incorrecta")
# ===============================
# 🔎 REVISAR CLAVE ÚNICA (SOLO ADMIN)
# ===============================
if es_admin:
    st.markdown("### 🔎 Revisar clave Premium")

    nombre_rev = st.text_input("Nombre completo a verificar", key="rev_nombre")
    fecha_rev = st.date_input(
        "Fecha de nacimiento a verificar",
        min_value=date(1936, 1, 1),
        max_value=date(2036, 12, 31),
        key="rev_fecha"
    )

    clave_rev = st.text_input(
        "Clave a verificar",
        type="password",
        key="rev_clave"
    )
    ...
    if st.button("🔍 Verificar clave"):
        if nombre_rev and fecha_rev and clave_rev:
            clave_correcta = generar_clave_unica(nombre_rev, fecha_rev)

            if clave_rev.strip().upper() == clave_correcta:
                st.success("✅ La clave ES VÁLIDA para esta persona")
            else:
                st.error("❌ La clave NO corresponde a estos datos")
        else:
            st.warning("⚠️ Completa todos los campos para verificar")

# ===============================
# 🔑 INGRESO DE CLAVE PREMIUM (CLIENTE)
# ===============================
...
# ===============================
# CONTADOR DE INTENTOS PREMIUM
# ===============================
if "intentos_premium" not in st.session_state:
    st.session_state["intentos_premium"] = 0
